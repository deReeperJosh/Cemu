...
namespace nsyshid
{
	...
	bool InfinityBaseDevice::SetProtocol(uint32 ifIndex, uint32 protocol)
	{
		return true;
	}
	...
	std::string InfinityUSB::FindFigure(uint32 figNum)
	{
		...
		return fmt::format("Unknown Figure ({})", figNum);
	}
	...
	bool InfinityUSB::CreateFigure(fs::path pathName, uint32 figureNum)
	{
		...
		std::array<uint8, 16> figureData = GenerateBlankFigureData(figureNum);
	...
	std::array<uint8, 16> InfinityUSB::GenerateBlankFigureData(uint32 figureNum)
	{
